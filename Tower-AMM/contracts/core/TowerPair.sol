...
import "./TowerLPToken.sol";
import "../interfaces/ITowerCallee.sol";
...
contract TowerPair is TowerLPToken {
    ...
    modifier lock() {
        require(unlocked == 1, "Tower: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "Tower: FORBIDDEN");
        require(token0 == address(0) && token1 == address(0), "Tower: ALREADY_INITIALIZED");
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "Tower: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, "Tower: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, "Tower: INSUFFICIENT_OUTPUT_AMOUNT");
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves();
        require(amount0Out < _reserve0 && amount1Out < _reserve1, "Tower: INSUFFICIENT_LIQUIDITY");
        ...
        require(to != _token0 && to != _token1, "Tower: INVALID_TO");
        ...
        if (data.length > 0) {
            ITowerCallee(to).towerCall(msg.sender, amount0Out, amount1Out, data);
        }
        ...
        require(amount0In > 0 || amount1In > 0, "Tower: INSUFFICIENT_INPUT_AMOUNT");
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ITowerFactory(factory).feeTo();
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, "Tower: OVERFLOW");
