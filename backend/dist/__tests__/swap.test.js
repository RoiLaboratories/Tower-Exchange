...
describe('SwapMathUtils', () => {
    describe('calculateMinimumOutputAmount', () => {
        it('should calculate minimum output with slippage', () => {
            const expectedAmount = ethers_1.ethers.utils.parseEther('100').toString();
            const slippageBps = 50; // 0.5%
            const result = helpers_1.SwapMathUtils.calculateMinimumOutputAmount(expectedAmount, slippageBps);
            const resultNum = parseFloat(ethers_1.ethers.utils.formatEther(result));
            const expectedNum = 100 * 0.995; // 100 - 0.5%
            expect(Math.abs(resultNum - expectedNum)).toBeLessThan(0.01);
        });
        it('should handle zero slippage', () => {
            const expectedAmount = ethers_1.ethers.utils.parseEther('100').toString();
            const result = helpers_1.SwapMathUtils.calculateMinimumOutputAmount(expectedAmount, 0);
            expect(result).toBe(expectedAmount);
        });
        it('should reject excessive slippage', () => {
            const expectedAmount = ethers_1.ethers.utils.parseEther('100').toString();
            expect(() => {
                helpers_1.SwapMathUtils.calculateMinimumOutputAmount(expectedAmount, 10001);
            }).toThrow();
        });
    });
    describe('calculateAmountOut', () => {
        it('should calculate output amount correctly', () => {
            const amountIn = ethers_1.ethers.utils.parseEther('1').toString();
            const reserveIn = ethers_1.ethers.utils.parseEther('1000').toString();
            const reserveOut = ethers_1.ethers.utils.parseEther('1000').toString();
            const result = helpers_1.SwapMathUtils.calculateAmountOut(amountIn, reserveIn, reserveOut, 25);
            const resultNum = parseFloat(ethers_1.ethers.utils.formatEther(result));
        ...
        it('should handle empty reserves', () => {
            ...
            const reserveOut = ethers_1.ethers.utils.parseEther('1000').toString();
            expect(() => {
                helpers_1.SwapMathUtils.calculateAmountOut(amountIn, reserveIn, reserveOut);
            }).toThrow();
        });
    });
    describe('calculateFee', () => {
        it('should calculate fee correctly', () => {
            const amount = ethers_1.ethers.utils.parseEther('1000').toString();
            const feeBps = 25; // 0.25%
            const fee = helpers_1.SwapMathUtils.calculateFee(amount, feeBps);
            const feeNum = parseFloat(ethers_1.ethers.utils.formatEther(fee));
            expect(Math.abs(feeNum - 2.5)).toBeLessThan(0.01); // 0.25% of 1000
        });
        it('should reject excessive fees', () => {
            const amount = ethers_1.ethers.utils.parseEther('1000').toString();
            expect(() => {
                helpers_1.SwapMathUtils.calculateFee(amount, 50); // > 30 bps
            }).toThrow();
        });
    });
});
describe('TokenUtils', () => {
    describe('toWei and fromWei', () => {
        it('should convert between units correctly', () => {
            const amount = '100.5';
            const decimals = 18;
            const wei = helpers_1.TokenUtils.toWei(amount, decimals);
            const back = helpers_1.TokenUtils.fromWei(wei, decimals);
...
describe('EncodingUtils', () => {
    describe('encodeApprove', () => {
        it('should encode approve call correctly', () => {
            ...
            const amount = ethers_1.ethers.constants.MaxUint256.toString();
            const encoded = helpers_1.EncodingUtils.encodeApprove(spender, amount);
    ...
    describe('encodeTowerRouterSwap', () => {
        it('should encode swap call correctly', () => {
            const amountIn = ethers_1.ethers.utils.parseEther('1').toString();
            const minAmountOut = ethers_1.ethers.utils.parseEther('0.99').toString();
            const path = [
...
describe('Integration Tests', () => {
    ...
    beforeAll(() => {
        ...
        provider = ethers_1.ethers.providers.getDefaultProvider('homestead');
    ...
    describe('RouteOptimizer', () => {
        it('should initialize with config', () => {
            ...
            expect(() => {
                new RouteOptimizer_1.RouteOptimizer(dexService, config);
            }).not.toThrow();
        });
    });
    describe('TransactionBuilder', () => {
        it('should validate transaction correctly', () => {
            ...
            const txBuilder = new TransactionBuilder_1.TransactionBuilder(config, provider);
            const validTx = {
