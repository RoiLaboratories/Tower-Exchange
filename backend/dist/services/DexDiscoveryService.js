...
exports.DexDiscoveryService = void 0;
...
class DexDiscoveryService {
     ...
     */
    getDexesSupportingPair(token0, token1) {
        ...
        return this.getAllDexes().filter((dex) => {
            if (dex.supportedTokens.length === 0)
                return true; // Support all if not restricted
            const supportedLower = dex.supportedTokens.map((t) => t.toLowerCase());
            return (supportedLower.includes(normalizedToken0) && supportedLower.includes(normalizedToken1));
        });
    }
    /**
    ...
    getTokenSymbol(tokenAddress) {
        const symbol = (0, tokenRegistry_1.getTokenSymbol)(tokenAddress);
        return symbol || `${tokenAddress.substring(0, 6)}...${tokenAddress.substring(-4)}`;
    }
    /**
     * Get Synthra pool address for a token pair
     */
    getSynthraPoolAddress(token0, token1) {
        const poolInfo = (0, SynthraPoolRegistry_1.getSynthraPoolForPair)(token0, token1);
        if (!poolInfo)
            return null;
     ...
     */
    getSynthraUniversalRouter() {
        return SynthraPoolRegistry_1.SYNTHRA_UNIVERSAL_ROUTER;
    }
    /**
    ...
    async getPoolReserves(dexId, token0, token1) {
        ...
        // Check cache first
        const cached = helpers_1.CacheUtils.get(cacheKey);
        if (cached)
            return cached;
        try {
            ...
            helpers_1.CacheUtils.set(cacheKey, poolData, 30 * 1000);
     ...
     */
    async getPrice(dexId, token0, token1) {
        const pool = await this.getPoolReserves(dexId, token0, token1);
        if (!pool || pool.reserve0 === '0' || pool.reserve1 === '0') {
            return null;
        }
        try {
            const reserve0 = ethers_1.ethers.BigNumber.from(pool.reserve0);
            const reserve1 = ethers_1.ethers.BigNumber.from(pool.reserve1);
     ...
     */
    async getBestPrice(token0, token1) {
     ...
     */
    async pairExists(dexId, token0, token1) {
        try {
            const pool = await this.getPoolReserves(dexId, token0, token1);
            return pool !== null;
        }
     ...
     */
    async getLiquidityDepth(dexId, token0, token1, amountIn) {
        try {
            const pool = await this.getPoolReserves(dexId, token0, token1);
            if (!pool)
                return null;
            const reserve0 = ethers_1.ethers.BigNumber.from(pool.reserve0);
            const amountInBn = ethers_1.ethers.BigNumber.from(amountIn);
...
exports.DexDiscoveryService = DexDiscoveryService;
//# sourceMappingURL=DexDiscoveryService.js.map